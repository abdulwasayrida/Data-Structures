...
public class List<T> { 
	...
	/**
	 * overrides the toString method for object to print the contents of the linked
	 * list to the screen separated by a blank space
	 * 
	 * @precondition
	 * @postcondition
	...
	public void advanceNTimes(int n) throws NullPointerException, IndexOutOfBoundsException {
		...
		for (int i = 0; i < n; i++) {
			if ((i + n) > length) {
				throw new IndexOutOfBoundsException("advanceNTimes(): Cannot advance! Iterator will be out of bounds! ");
			}
			advanceIterator();
		}
